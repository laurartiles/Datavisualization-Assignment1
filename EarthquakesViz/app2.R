...
earthquakes$Year<-format(as.Date(earthquakes$Date, format="%m/%d/%Y"),"%Y")
earthquakes$Month<-format(as.Date(earthquakes$Date, format="%m/%d/%Y"),"%m")
earthquakes$Day<-format(as.Date(earthquakes$Date, format="%m/%d/%Y"),"%d")
...
#Convert Long Lat to Country Names
world <- map('world', fill=TRUE, col="transparent", plot=FALSE)
IDs <- sapply(strsplit(world$names, ":"), function(x) x[1])
world_sp <- map2SpatialPolygons(world, IDs=IDs,
                                proj4string=CRS("+proj=longlat +datum=WGS84"))
pointsSP <- SpatialPoints(cbind(x = earthquakes$Longitude, y= earthquakes$Latitude),
                          proj4string=CRS("+proj=longlat +datum=WGS84"))
indices <- over(pointsSP, world_sp)
stateNames <- sapply(world_sp@polygons, function(x) x@ID)
earthquakes$Country <- stateNames[indices]

earthquakes_country <- earthquakes[!is.na(earthquakes$Country),]
...
server <- function(input, output) {
    ...
    get.used.earthquakes2 <- reactive({ 
      filter(earthquakes, 
             Date >= input$dateRange2[1], 
             Date <= input$dateRange2[2], 
             Magnitude >= input$magnitudeRange2[1], 
             Magnitude <= input$magnitudeRange2[2],
             Continent %in% input$continents2
      )})
